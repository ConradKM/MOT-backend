...
def _book_mot(session, garage, vehicle, customer, appt_type, *, start_days, status="BOOKED"):
    start = datetime.datetime.combine(_expiry_in(start_days), datetime.time(10, 0), tzinfo=UTC)
    appt = Appointment(
        garage_id=garage.id,
...
def test_owner_can_disable_a_stage(authenticated_client, garage):
    resp = authenticated_client.put("/api/mot-reminders/settings", json={"stage2_enabled": False})
    assert resp.status_code == 200
...
def test_schedule_is_tenant_isolated(authenticated_client, second_authenticated_client):
    authenticated_client.put("/api/mot-reminders/settings", json={"stage1_days_before": 45})
...
def test_first_reminder_sends_once_and_only_once(session, garage, vehicle, customer, mot_record):
...
def test_expired_mot_gets_no_pre_expiry_reminders(session, garage, vehicle, customer, mot_record):
    _set_expiry(session, vehicle, -1)
    assert _run_worker(session, garage) == []


def test_customer_with_no_booking_stays_eligible(session, garage, vehicle, customer, mot_record):
...
def test_booking_a_different_vehicle_does_not_suppress(session, garage, customer, appointment_type):
...
def test_cancelled_booking_restores_eligibility(
    ...
    _set_expiry(session, vehicle, 20)
    appt = _book_mot(session, garage, vehicle, customer, appointment_type, start_days=5)
    assert _run_worker(session, garage) == []
...
def test_row_shape_and_next_scheduled(
    ...
    assert (
        row["next_reminder_scheduled"]
        == (vehicle.mot_expiry_date - datetime.timedelta(days=30)).isoformat()
    )
...
def test_manual_send_rejects_a_foreign_vehicle(authenticated_client, second_vehicle):
    resp = authenticated_client.post(f"/api/mot-reminders/{second_vehicle.id}/send", json={})
