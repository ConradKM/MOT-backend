...
# How the caller reached the assistant (app/ai_voice/sip_handoff.py). DIRECT
# is the Elastic SIP trunk path, with no phone menu in front of it.
ROUTE_DIRECT = "DIRECT"
ROUTE_BOOKING = "AI_BOOKING"
ROUTE_FAQ = "AI_FAQ"

_ROUTE_CONTEXT = {
    ROUTE_BOOKING: (
        "The caller has just chosen the bookings option from this business's phone menu, so "
        "they have already heard the business's greeting. Focus on getting them booked in.\n\n"
    ),
    ROUTE_FAQ: (
        "The caller has just chosen the questions option from this business's phone menu, so "
        "they have already heard the business's greeting. Answer their questions; if they then "
        "want to book, help them book.\n\n"
    ),
}


def build_instructions(garage, *, now: datetime | None = None, route: str = ROUTE_DIRECT) -> str:
    ...
    return (
        _date_context(garage, now)
        + _ROUTE_CONTEXT.get(route, "")
        + (
            ...
            "Hand the caller to a person with request_human_handoff - never guess or improvise to "
            "avoid it - when: they ask for a person or don't want to use the assistant; you still "
            "can't understand them after a couple of attempts; a booking, lookup, cancellation or "
            "payment step fails or can't be completed; they need something your tools don't cover; "
            "the information they need isn't available from your tools; or you're unsure of the "
            "right answer. Say you'll try to put them through to the team - never that they're "
            "connected, or that someone will definitely answer.\n\n"
...
def build_greeting_instructions(garage, *, route: str = ROUTE_DIRECT) -> str:
    ...
    if route == ROUTE_BOOKING:
        return (
            "In one short sentence, say you can help them book an appointment and ask what "
            "they would like to book. Do not thank them for calling again."
        )
    if route == ROUTE_FAQ:
        return (
            "In one short sentence, ask what they would like to know. Do not thank them for "
            "calling again."
        )
    return (
