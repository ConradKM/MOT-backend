...
def upgrade():
    op.execute(
        "UPDATE garage_schedule_settings SET min_lead_time_hours = 2 WHERE min_lead_time_hours = 24"
    )


def downgrade():
    op.execute(
        "UPDATE garage_schedule_settings SET min_lead_time_hours = 24 WHERE min_lead_time_hours = 2"
    )
